    ...
    <header>
        <div class="container py-3 mb-2 border-bottom">
            @auth
                ...
                @endif
                <span class="alert float-start "><b class="shadow-sm">{{ auth()->user()->name }}</b> <a href="{{ route('profile.index') }}" title="Посмотреть профиль"><img class="avatar-mini" src="{{ asset('storage/img/users') }}/{{ auth()->user()->picture }}" alt="Аватар"></a> </span>
            @endauth
